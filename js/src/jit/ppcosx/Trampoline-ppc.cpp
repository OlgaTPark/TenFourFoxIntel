...
JitRuntime::generateInvalidator(JSContext *cx)
{
    ...
    // XXX We don't have to push everything, but this is simpler for now
    // and makes us congruent with ARM, our closest supported architecture.
    // Besides, this operation is expensive anyway.
    // If we change this, we have to change the IonFrame too!
    for (uint32_t i=0; i < Registers::Total; i++)
        masm.stwu(Register::FromCode(i), stackPointerRegister, -4);
    for (uint32_t i=0; i < FloatRegisters::Total; i++)
        masm.stfdu(FloatRegister::FromCode(i), stackPointerRegister, -8);

    // Call InvalidationBailout with on-stack pointers for our retvals.
    ...
    masm.x_mr(r5, stackPointerRegister);
    ...
    // InvalidationBailout returns on the stack, not in argregs (wtf).
    masm.lwz(r5, stackPointerRegister, 0);
    masm.lwz(r4, stackPointerRegister, sizeOfBailoutInfo);
    ...
    masm.addi(stackPointerRegister, stackPointerRegister,
        sizeof(InvalidationBailoutStack) + sizeOfRetval + sizeOfBailoutInfo);
    // Remove everything else (InvalidationBailout tells us how much).
...
JitCode *
JitRuntime::generateArgumentsRectifier(JSContext *cx, ExecutionMode mode,
    void **returnAddrOut)
{
    MacroAssembler masm(cx);
    ...
    JS_ASSERT(JSFunction::offsetOfNargs() < 65536);
    masm.lhz(r8, r4, JSFunction::offsetOfNargs());
...
{
    // Upon call, the stack looks like this.
    // [IonFrame]
    // bailoutFrame.registersnapshot
    // bailoutFrame.fpsnapshot
    // bailoutFrame.snapshotOffset
                     
    // Construct the BailoutStack (see Bailouts-ppc.cpp).
    // Save our register sets to the stack so Bailout() can read everything. 
    // XXX We don't have to push everything, but this is simpler for now
    // and makes us congruent with ARM, our closest supported architecture.
    // Besides, this operation is expensive anyway.
    // If we change this, we have to change MachineState and the IonFrame too!
    for (uint32_t i=0; i < Registers::Total; i++)
        masm.stwu(Register::FromCode(i), stackPointerRegister, -4);
    for (uint32_t i=0; i < FloatRegisters::Total; i++)
        masm.stfdu(FloatRegister::FromCode(i), stackPointerRegister, -8);
    // XXX When we change these, fix bailoutFrameSize below.

    masm.x_mflr(addressTempRegister); // new dispatch group
    masm.x_li32(tempRegister, frameClass);
    if (frameClass != NO_FRAME_SIZE_CLASS_ID) { // Frame has a classy size
        // If the frameSize is implied by the frameClass, LR needs to be
        // wound forward and pushed as a table offset.
        masm.addi(addressTempRegister, addressTempRegister,
            PPC_CALL_STANZA_LENGTH - 4);
    }
    else {
        // XXX Assertion I should write for debug mode:
        // if frameClass is -1, then addressTempRegister should contain
        // some reasonable framesize, certainly less than 64K.
    }
    masm.addi(stackPointerRegister, stackPointerRegister, -8);
    ...
    masm.x_mr(r3, stackPointerRegister);    // BailoutStack
    const int sizeOfBailoutInfo = sizeof(void *)*2;
    masm.reserveStack(sizeOfBailoutInfo);   // BaselineBailoutInfo
    masm.x_mr(r4, stackPointerRegister);
    ...
    // Return value is on the stack. Ape ARM. Leave r3 alone;
    // generateBailoutTail() uses it also.
    masm.lwz(r5, stackPointerRegister, 0);

    // Remove the BailoutInfo frame and the registers.
    uint32_t bailoutFrameSize =
        sizeof(void *) +
        sizeOfBailoutInfo +
        sizeof(double) * FloatRegisters::Total +
        sizeof(void *) * Registers::Total;
    JS_ASSERT(bailoutFrameSize < 32767);
    ...
    if (frameClass == NO_FRAME_SIZE_CLASS_ID) { // Frame has a tacky size
        ...
        masm.addi(stackPointerRegister, stackPointerRegister,
            bailoutFrameSize);
        masm.lwz(addressTempRegister, stackPointerRegister, 0); // frameSize
        masm.add(stackPointerRegister, stackPointerRegister,
            addressTempRegister);
        // Snapshot is now on top.
masm.x_trap();
    } else {
        // Stack is:
        //    ... frame ...
        ...
        bailoutFrameSize += frameSize + sizeof(void *); // bailoutId
        JS_ASSERT(bailoutFrameSize < 32767);
        masm.addi(stackPointerRegister, stackPointerRegister,
            bailoutFrameSize);
masm.x_trap();
...
JitRuntime::generateBailoutTable(JSContext *cx, uint32_t frameClass)
{
    ...
    for (size_t i = 0; i < BAILOUT_TABLE_SIZE; i++)
        masm.call(&bailout); // indirect branch
    masm.bind(&bailout);
...
/* The VMWrapper is considered JitCode and MUST be called with callIon, or
...
JitRuntime::generateVMWrapper(JSContext *cx, const VMFunction &f)
{
    ...
    switch (f.outParam) {
      ...
      case Type_Double:
        masm.loadDouble(Address(stackPointerRegister, 0), ReturnFloatReg);
...
JitRuntime::generateExceptionTailStub(JSContext *cx, void *handler)
{
    ...
    masm.handleFailureWithHandlerTail(handler); // Its name is Eric.
