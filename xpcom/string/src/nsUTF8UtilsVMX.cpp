...
void
LossyConvertEncoding16to8::write_vmx(const PRUnichar* aSource,
                                      uint32_t aSourceLength)
{
  ...
  if (aSourceLength >= alignLen && aSourceLength - alignLen > 31) {
    ...
    if (maxIndex <= aSourceLength && i < maxIndex) {
      const PRUnichar *aOurSource = &aSource[i];
                                      ...
                                      uint32_t aSourceLength)
{
  ...
  // We must use unsigned datatypes because aSourceLength is unsigned.
  uint32_t i = 0;
  uint32_t alignLen = XPCOM_MIN<uint32_t>(aSourceLength, uint32_t(-NS_PTR_TO_INT32(dest) & 0xf) / sizeof(PRUnichar));
  // subtraction result can underflow if aSourceLength < alignLen!!!
  // check for underflow
  if (aSourceLength >= alignLen && aSourceLength - alignLen > 31) {
    for (; i < alignLen; i++) {
