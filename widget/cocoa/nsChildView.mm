...
// XXX: This can eventually be removed since it just stubs into the above
// after bug 974318.
static void
FillTextRangeInTextEvent(WidgetTextEvent *aTextEvent, NSAttributedString* aString, NSRange markRange, NSRange selRange)
{
  ...
  // Allocate the right size of TextRange and draw caret at right position.
  // Convert the attributed string into an array of TextRange and get current caret position by calling above functions.
  aTextEvent->mRanges = ConvertAttributeToGeckoRange(aString,
...
// Use the Mozilla 7 code.
  ...
  for (uint32_t i = 0; i < MOZ_ARRAY_LENGTH(sModifierFlagMap); ++i) {
...
#else
    // Boost the alpha for TenFourFox.
    [[NSColor colorWithDeviceWhite:1.0 alpha:0.9 * (1.0 - t)] set];
#endif
...
#else
  if (wheelEvent.deltaMode == nsIDOMWheelEvent::DOM_DELTA_PIXEL) {
    // Scale factor is always 1 for Power Macs and PowerBook/iBook
    // trackpad scrolling (issue 283).
                        ...
                        toGeckoEvent:(WidgetWheelEvent*)outWheelEvent
{
  ...
  // Since this can be enabled for 10.5, I guess we have to support it.
  // http://www.macworld.com/article/1144054/magicmousescroll.html
...
// Note: DO NOT USE for NS_COMPOSITION_UPDATE -- see sendTextEvent.
...
- (void)sendCompositionEvent:(int32_t) aEventType
{
  ...
  NS_ASSERTION(aEventType != NS_COMPOSITION_UPDATE,
	"Does not handle NS_COMPOSITION_UPDATE yet");
                      ...
                      doCommit:(BOOL) doCommit
{
  ...
  WidgetTextEvent textEvent(true, NS_TEXT_TEXT, mGeckoChild);
  textEvent.time = PR_IntervalNow();
  textEvent.theText = aBuffer;
  if (!doCommit)
    FillTextRangeInTextEvent(&textEvent, aString, markRange, selRange);

  // Do everything here that TextInputHandler's DispatchTextEvent does.
  if (textEvent.theText != mLastDispatchedCompositionString) {
    // XXX. sendCompositionEvent does not yet handle this.
    WidgetCompositionEvent uEvent(true, NS_COMPOSITION_UPDATE, mGeckoChild);
    uEvent.time = textEvent.time;
    uEvent.data = textEvent.theText;
    mLastDispatchedCompositionString = textEvent.theText;
    mGeckoChild->DispatchWindowEvent(uEvent);
  }

  if (!mGeckoChild) return; // paranoia
...
// It's 10.5 only, so we don't implement it. See bug 875674.
...
{
// Since plugins cannot execute, this is now a no-op.
#if(0)
